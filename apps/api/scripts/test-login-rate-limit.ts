...
test('HTTP: 10 failed logins → 401s, 11th → 429', async () => {
  ...
  try {
    // 10 attempts → expect 401 (rate limiter passes the request through).
    for (let i = 0; i < 10; i++) {
      const res = await makeJsonRequest(ts.serverUrl, 'POST', '/auth/login', invalidBody, headers);
      assert.equal(res.statusCode, 401, `Attempt ${i + 1}: expected 401, got ${res.statusCode}`);
    }

    // 11th → 429
    const eleventh = await makeJsonRequest(ts.serverUrl, 'POST', '/auth/login', invalidBody, headers);
